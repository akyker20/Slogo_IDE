...
public abstract class StructuralCommand extends CommandParser {
    ...
    protected void checkForOpeningBrace (Iterator<String> commandStringIterator)
        ...
        if (!workspace.translator.matchesListStartPattern(stringOfInterest)) { throw new CompileTimeParsingException(
    ...
    protected void checkForClosingBrace (Iterator<String> commandStringIterator)
            throws CompileTimeParsingException {
        String stringOfInterest = commandStringIterator.next();
        if (!workspace.translator.matchesListEndPattern(stringOfInterest)) { throw new CompileTimeParsingException(
    ...
    protected void extractCommandsBetweenBraces (Iterator<String> commandStringIterator)
        ...
        if (!commandStringIterator.hasNext() &&
                !workspace.translator.matchesListEndPattern(stringOfInterest)) { throw new CompileTimeParsingException(
                        "expected closing brace"); }

        enclosedCommands = commandList;
    }

    protected void parseCommandsBetweenBraces (Iterator<String> commands,
                                               Queue<DrawableObject> objectQueue)
                                                       throws SLOGOException {
        double value = 0;
        while (commands.hasNext()) {
            String stringOfInterest = commands.next();
            CommandParser parser = createParser(stringOfInterest, workspace);
    ...
    protected void ignoreUntilClosingBrace (Iterator<String> commandStringIterator)
        ...
        if (!stoppedParsingBeforeEndOfString) { throw new CompileTimeParsingException(
                                                                                      "expected closing brace"); }
    }
    ...
    protected String getVariable (Iterator<String> commandStringIterator,
                                  Queue<DrawableObject> objectQueue)
                                          throws SLOGOException {
